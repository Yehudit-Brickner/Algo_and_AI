...
import dom_parser.Node_mat.Node_mat_Comparator;
...
public class Ex1 {
	...
	public static String bayesBall(Nodes n1, Nodes n2 ,ArrayList<Nodes> evidence, ArrayList<Nodes> nodesarr){
		...
		bayesBalldown (n1, n2, isdependent, seenE);
		bayesBallup (n1, n2, isdependent, seenE);
	...
	public static String  variableElimination1(String s, String[] given, String[] hidden, ArrayList<Nodes> nodearr) {
		...
		Node_mat_Comparator comp= new Node_mat_Comparator();
		int count=0;
		...
		if (!is_Q_or_g) {
			for (int j=0; j<nodearr.size();j++) {
				if(nodearr.get(j).getName().equals(Qwry)) {
					...
					isIndependent=bayesBall(n1,node,given_nodes,nodearr);
	...
	public static Object[][] join_matrix(Object[][] mat_smaller, Object[][] mat_bigger, String s,ArrayList<Nodes> nodearr) {
		...
		for (int i=1;i<new_mat.length;i++) { //rows
			truth_val_row.clear();
			for (int j=0; j<col_small.size();j++) {
				truth_val_row.add((String)new_mat[i][col_small.get(j)]);
			}
			added=false;
			for (int k=1;k<mat_smaller.length &&!added ;k++) { // rows
				wrong=false;
				for (int l=0; l<col_small.size()&& !wrong;l++){
					if(!truth_val_row.get(l).equals(mat_smaller[k][l])) {
						wrong=true;
					}
				}
				if (!wrong) {
	...
	/*
		for (int i=0; i< n1.kids.size(); i++) {
			if(n1.kids.get(i).getState()==2) {
				break;
			}
			else if (n1.kids.get(i).getState()==0) {
				n1.kids.get(i).setState(1);
			}
			else {
				n1.kids.get(i).setState(2);
			}

			if (n1.seen==false && n1.kids.get(i).getSeen()==false) {
				...
				if (n1.kids.get(i).equals(n2)) {
					...
					bayesBall1 (n1.kids.get(i),n2,isdependent,seenE);

				}
			}
			// parent to child
			// parent is not seen child is not seen- 
			// parent can only go to children
			//}

			else if (n1.seen==false && n1.kids.get(i).getSeen()==true) {
				// if going from parent to child
				seenE=true;
				for (int j=0; j<n1.kids.get(i).parents.size();j++) {
					if(n1.kids.get(i).parents.get(j).getState()==2) {
						break;
					}
					else if (n1.kids.get(i).parents.get(j).getState()==0) {
						n1.kids.get(i).parents.get(j).setState(1);
					}
					else {
						n1.kids.get(i).parents.get(j).setState(2);
					}
					...
					if (n1.kids.get(i).parents.get(j).equals(n2)) {
						...
						bayesBall1 (n1.kids.get(i).parents.get(j),n2,isdependent,seenE);
					}
				}
				// parent to child
				// parent is not seen child is seen- 
				// parent can only go to other parent



				// child to parent
				// parent is not seen child is not seen- 
				// child can go to siblings or parents 
			}

			else if (n1.kids.get(i).getSeen()==false && n1.seen==true) {
				isdependent.add(false);

				// child to parent
				// parent is seen child is not seen-
				// child can't go any where
			}

			else if(n1.kids.get(i).getSeen()==true && n1.seen==true) {
				isdependent.add(false);
				// child to parent
				// parent is seen kid is seen
				// child can't go anywhere	
			}
		}
		if (seenE==true) {
			for (int i=0; i< n1.parents.size(); i++) {
				if(n1.parents.get(i).getState()==2) {
					break;
				}
				else if (n1.parents.get(i).getState()==0) {
					n1.parents.get(i).setState(1);
				}
				else {
					n1.parents.get(i).setState(2);
				}

				if (n1.seen==false && n1.parents.get(i).getSeen()==false) {
					if (n1.parents.get(i).equals(n2)) {
						isdependent.add(true);
					}
					else {

						bayesBall1 (n1.parents.get(i),n2,isdependent,seenE);

					}
				}
				else if (n1.seen==false && n1.parents.get(i).getSeen()==true) {
					// if going from parent to kid
